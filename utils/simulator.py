...
from utils.dataset import MyTextLoader
import numpy as np
...
class Simulator:
    ...
    def run(self):
        ...
        if self.args.dataset != 'wiki_text2':
            ...
            train_set = train_set[:len(train_set)//3]
            length = len(train_set) // self.args.client_num
            train_sets = [train_set[length * i: length * (i + 1)] for i in range(self.args.client_num)]
        ...
        for k in client_pool[0].model.state_dict().keys():
            print(k + str('   ') + str(client_pool[0].model.state_dict()[k].numel()))
        ...
        if self.args.dataset == 'wiki_text2':
            server = Server(self.args.device, MyTextLoader(test_set))
