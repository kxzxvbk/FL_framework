...
from utils.compressor import comp_cond
...
class Client:
    def __init__(self, train_dataset, args, client_id, test_dataset=None):
        ...
        self.nlp = args.dataset == 'wiki_text2'
            ...
            for _, (batch_x, batch_y) in enumerate(self.train_dataloader):
                ...
                if self.nlp:
                    ...
                    if batch_size != 64:  # only on last batch
                        continue
                    o = self.model(batch_x, src_mask).view(-1, 28783)
                else:
